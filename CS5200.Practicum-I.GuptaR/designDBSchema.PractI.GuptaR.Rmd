...
```{r setup, echo=FALSE, warning=FALSE, eval=FALSE, message=FALSE}
# Setup the markdown so that none of the code chunks or their outputs are shown
# in the knitted file (since that needs to only contain the relations from the
# decomp)
library(knitr)
...
db <- read.csv("restaurant-visits-139874.csv")
...
```{r ExploreNAs, echo=FALSE, eval=FALSE}
...
```{r checkFDs, echo=FALSE, eval=FALSE, message=FALSE, warning=FALSE}
...
```{r checkMtoM, echo=FALSE, eval=FALSE}
...
```{r catCols, echo=FALSE, eval=FALSE}
...
```{r genderLevels, echo=FALSE, eval=FALSE, warning=FALSE, message=FALSE}
# Load for str_split()
library(stringr)
...
# after submission
knitr::include_graphics("PracticumI-ERD.pdf")
```
...
- **Summary:** The main table containing all info for each individual restaurant visit.
  ...
  - **FKs:**
    ...
    - *ServerAssignmentID* &rarr; RestaurantServers (NULLABLE to accommodate take-out orders)
